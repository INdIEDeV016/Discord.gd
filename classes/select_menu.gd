...
var custom_id: String:
	set = set_custom_id, get = get_custom_id
var placeholder: String:
	set = set_placeholder, get = get_placeholder
var options: Array:
...
var disabled: bool = false:
	set = set_disabled, get = get_disabled

var type: int = 3


func set_custom_id(new_custom_id: String):
	assert(new_custom_id.length() <= 100, 'custom_id of SelectMenu cannot be more than 100 characters.')
...
func add_option(value: String, label: String, data: Dictionary = {}):
	...
	if data.has('description'):
		...
		assert(data.description.length() <= 100, 'description of SelectMenu cannot be more than 100 characters')
...
func set_placeholder(new_placeholder: String):
	assert(new_placeholder.length() <= 100, 'placeholder of SelectMenu cannot be more than 100 characters.')
